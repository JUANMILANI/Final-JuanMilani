...
const getAll = async() => {
    const query = "SELECT p.id, p.nombre, p.descripcion, p.precio, p.stock, c.nombre AS nombreCategoria, pi.uid FROM ?? AS p JOIN ?? AS c ON p.id_categoria = c.id JOIN ?? as pi ON p.id = pi.id_producto WHERE p.eliminado = 0";
...
const getSingle = async(id) => {
    const query = "SELECT p.id, p.nombre, p.descripcion, p.precio, p.stock, p.id_categoria, c.nombre AS nombreCategoria, pi.uid FROM ?? AS p JOIN ?? AS c ON p.id_categoria = c.id JOIN ?? as pi ON p.id = pi.id_producto WHERE p.id = ? AND p.eliminado = 0";
